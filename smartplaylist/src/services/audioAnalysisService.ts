...
export class AudioAnalysisService {
  ...
  /**
   * Analyzes audio features using AI
   */
  async analyzeAudio(audioUrl: string): Promise<AudioFeatures> {
    try {
      // Use Groq for initial analysis
      const prompt = `Analyze the audio features of this song: ${audioUrl}
        Please provide:
        - BPM (tempo)
        - Musical key
        - Mode (major/minor)
      ...
      const initialAnalysis = JSON.parse(response) as AudioFeatures;
